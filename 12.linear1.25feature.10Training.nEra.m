...
lastnEras=Table[With[{n=h},trainPureData[[cummulativeEraSizes[[-1-n]]+1;;cummulativeEraSizes[[-1]]]]],{h,1,5,1}];
...
(**)
(*(*Last 3 months, 6 months, 9 months, 12 months, 15, 18, 21, 24 months*)*)
nErasVal=Table[With[{n=h},valPureData[[1;;cummulativeEraSizesVal[[h]]]]],{h,1,2,1}];
nErasVal//Dimensions
Print["nErasVal: "]
...
(*methods={"LinearRegression","NearestNeighbors","RandomForest"};*)
...
Clear[q,topTenVal]
results={}; raw={};
For[q=0,q<2,q++; valSetSize=Dimensions[nErasVal[[q]]][[1]]; NumeraiValidationData=nErasVal[[q]]; topTenVal=Transpose[NumeraiValidationData][[bestCorrs]]; trainInputVal=Transpose[topTenVal[[2;;-1]]]; trainOutputVal=topTenVal[[1]];  f[arg_]:=arg[[1]]->arg[[2]]; trainSetVal=Table[f[{trainInputVal[[i]],trainOutputVal[[i]]}],{i,1,Length[trainInputVal]}];  netTrainOutputVal=Table[Table[net[[j]][trainInputVal[[i]]],{i,1,Length@trainInputVal}],{j,1,Length@net}];  actualTrainVal=Table[{i,trainOutputVal[[i]]},{i,1,Length[trainInputVal]}]; netTrainVal=Table[Table[{i,netTrainOutputVal[[j]][[i]]},{i,1,Length[trainInputVal]}],{j,1,Length@net}];  trueResults=Transpose[actualTrainVal][[2]]; predictedResults=Table[Transpose[netTrainVal[[i]]][[2]],{i,1,Length@net}]; Clear[logloss]; loglossfun[binary_,prediction_]:=Table[-(binary[[i]]*Log[prediction[[i]]]+(1-binary[[i]])*Log[1-prediction[[i]]]),{i,1,Length@binary}];  logloss=Table[Mean@loglossfun[trueResults,predictedResults[[i]]],{i,1,Length@net}]; loglossSD=Table[StandardDeviation@loglossfun[trueResults,predictedResults[[i]]],{i,1,Length@net}]; loglossAccuracy=Table[StandardDeviation@loglossfun[trueResults,predictedResults[[i]]]/Sqrt[Length@predictedResults[[i]]],{i,1,Length@net}]; loglossRS=Table[Mean@DeleteCases[loglossfun[trueResults,Rescale[Standardize[predictedResults[[i]]]]],Indeterminate],{i,1,Length@net}]; loglossSDRS=Table[StandardDeviation@DeleteCases[loglossfun[trueResults,Rescale[Standardize[predictedResults[[i]]]]],Indeterminate],{i,1,Length@net}]; loglossAccuracyRS=Table[StandardDeviation@DeleteCases[loglossfun[trueResults,Rescale[Standardize[predictedResults[[i]]]]],Indeterminate]/Sqrt[Length@predictedResults[[i]]],{i,1,Length@net}];  polar2[f_]:=Table[If[f[[i]]>Mean[f],1,0],{i,1,Length@f}]; polarisedOutputVal2=Table[polar2@netTrainOutputVal[[i]],{i,1,Length[net]}]; jobsize={"Train-set size: ",trainSetSize," Validation-set size: ",valSetSize}; labels={{"Method","correlation","accuracy","logloss","logloss-error","RS-logloss","RS-logloss-error"}}; scores=Join[labels,Table[{methods[[i]],Correlation[trainOutputVal,polarisedOutputVal2[[i]]],(Correlation[trainOutputVal,polarisedOutputVal2[[i]]]+1)/2//N,logloss[[i]],loglossAccuracy[[i]],loglossRS[[i]],loglossAccuracyRS[[i]]},{i,1,Length@methods}]//N]; jobScoresRaw={{q},jobsize,{timings},{"Best features: ",ToString@topfeatures},scores}; AppendTo[raw,jobScoresRaw]; jobScores={{q}[[1]]//TableForm,jobsize//TableForm,{timings}[[1]]//TableForm,{"Best features: ",topfeatures}//TableForm,scores//TableForm}; AppendTo[results,jobScores];numeraiTest=results//TableForm; Print@numeraiTest[[-1]][[-1]] ]
